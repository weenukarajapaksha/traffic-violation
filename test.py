            ...
            for plate_box in plate_results[0].boxes:
                ...
                for result in ocr_result:
                    ...
                    if len(cleaned_text) >= 4:
                        ...
                        filename = f"violations_{detected_plate_text}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"

                        full_image_path = os.path.join("static", "violations", filename)

                        cv2.imwrite(full_image_path, frame)

                        # save only this to database
